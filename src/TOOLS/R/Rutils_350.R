...
mynd=function(file,grid='regu'){
  ...
  if (grid=='regu') {
    ...
    time=floor(as.numeric(nc$dim$time$vals)); timeu=nc$dim$time$units
    lon=ncvar_get(nc,nc$var[['longitude']])
    lonu = nc$var[['longitude']]$units
    lat=ncvar_get(nc,nc$var[['latitude']])
...
mynt=function(file,variable='ts'){
  nc=nc_open(file)
  values = ncvar_get(nc,nc$var[[variable]])
  units  = nc$var[[variable]]$units
  if ('time_counter' %in% names(nc$var)) {
   time   = ncvar_get(nc,'time_counter')
   timeu  = ncatt_get(nc,'time_counter','units')$value
   timec  = ncatt_get(nc,'time_counter','calendar')$value
  } else if ('time' %in% names(nc$var)) {
   time   = ncvar_get(nc,'time')
   timeu  = nc$var$time$units
   timec  = ncatt_get(nc,'time','calendar')$value
  } else if ('TIME' %in% names(nc$var)) {
   time   = ncvar_get(nc,'TIME')
   timeu  = nc$var$TIME$units
   timec  = ncatt_get(nc,'TIME','calendar')$value
...
mynt3D=function(file,variable='soicecov',latid='lat',lonid='lon',period=NULL){
  ...
  if (is.null(period)) {
    ...
    values = ncvar_get(nc,nc$var[[variable]],start=c(1,1,startt),count=c(dimvar[1],dimvar[2],countt))
...
which_mth2d=function(tab){
  sapply(tab, which_mth) 
}
...
to.date = function(t,timeu,cal='gregorian'){
  tdate = as.POSIXct.numeric(t,cal=cal,origin=timeorigin(timeu))
  tdate
}
...
annualmean=function(serie,option='average'){
  Ndim = length(dim(serie$values))
  years = unique(sapply(as.character(to.date(serie$time,serie$timeu,cal=serie$timec)),which_yr))
  if (Ndim==1){    
