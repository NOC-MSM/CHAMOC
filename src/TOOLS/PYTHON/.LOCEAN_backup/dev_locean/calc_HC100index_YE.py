...
depthindicemin, depthindicemax = ct.coord2indice(depth,(0,100))
...
if os.path.exists(fileout) :
  ...
  if 'HC100_AMO' in fileo.variables.keys() :
  ...
  for dom in domains :
    hc[dom] = fileo.variables['HC100_%s'%dom]
else :
  ...
  for dom in domains :
    hc[dom] = fileo.createVariable('HC100_%s'%dom,'f',('time',))
    hc[dom].units = '1e22 J'
    hc[dom].long_name = 'Heat content [0-100m] in %s'%dom
