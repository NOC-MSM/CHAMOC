...
if os.path.exists(fileout) :
  ...
  amo = fileo.variables['SST_NINO34']
  #amorev = fileo.variables['AMOrev']
  times = fileo.variables['time_counter']
  if 'SST_NINO34' in fileo.variables.keys() :
  ...
  fileo.comment = 'The index is computed as the spatial averaged of the annual SST in the NINO34 domain'
  time = fileo.createDimension('time',None)
  times = fileo.createVariable('time_counter','f',('time',))
  amo = fileo.createVariable('SST_NINO34','f',('time',))
  #amorev = fileo.createVariable('AMOrev','f',('time',))
  amo.units = 'degC'
  #amorev.units = 'degC'
  amo.long_name = 'NINO3.4 index'
...
for year in range(yrdeb,yrfin+1) :
  ...
  #print timex.year()
  #
  tab_amo = ct.regional_average(tab,'NINO34',operation='average')
  #
  # Annual mean
