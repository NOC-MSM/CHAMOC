...
depthindicemin, depthindicemax = ct.coord2indice(depth,(1800,2200))
...
if os.path.exists(fileout) :
  ...
  if 'HC1830_AMO' in fileo.variables.keys() :
  ...
  for dom in domains :
    hc[dom] = fileo.variables['HC1830_%s'%dom]
else :
  ...
  for dom in domains :
    hc[dom] = fileo.createVariable('HC1830_%s'%dom,'f',('time',))
    hc[dom].units = '1e22 J'
    hc[dom].long_name = 'Heat content [1800-2200 m] in %s'%dom
