...
if len(sys.argv)<3 :
  ...
  fin = sys.argv[2]
#
DIAG_ID = 'OMLMAX_MO_ATLboxlat'

# Repertoires des inputs :
# ~~~~~~~~~~~~~~~~~~~~~~~
exp = dexp[EXP_ID]
domains = ['ATL0010N','ATL1020N','ATL2030N','ATL3040N','ATL4050N','ATL5060N']

filein = exp.locfile('omlmax',realm='O')
#
...
if os.path.exists(fileout) :
  ...
  if 'OMLMAXmax_ATL0010N' in fileo.variables.keys() :
  ...
  for dom in domains :
    MLDmean[dom] = fileo.variables['OMLMAXmean_%s'%dom]
    MLDmax[dom]  = fileo.variables['OMLMAXmax_%s'%dom]
else :
  ...
  fileo.comment = 'The OMLMAXmax and OMLMAXmean are computed as the maximum and the average of omlmax in the domain'
  time = fileo.createDimension('time',None)
  times = fileo.createVariable('time_counter','f',('time',))
  for dom in domains :
    MLDmean[dom] = fileo.createVariable('OMLMAXmean_%s'%dom,'f',('time'))
    MLDmean[dom].units = 'm'
    MLDmean[dom].long_name = 'mixed layer thickness averaged over %s'%dom
    MLDmean[dom].domain = '%s in mask_ORCA2.nc'%dom
    MLDmax[dom] = fileo.createVariable('OMLMAXmax_%s'%dom,'f',('time'))
    MLDmax[dom].units = 'm'
...
for year in range(yrdeb,yrfin+1) :
  ...
  tab,timex = ct.read(filein,'omlmax',years=range(year,year+1))
  #
