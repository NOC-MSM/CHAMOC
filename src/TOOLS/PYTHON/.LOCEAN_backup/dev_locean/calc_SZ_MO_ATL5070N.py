...
filein = exp.locfile('vosaline',realm='O')
#
...
if os.path.exists(fileout) :
  ...
  if 'SZ_ATL5070N' in fileo.variables.keys() :
  ...
  for dom in domains :
    hc[dom] = fileo.variables['SZ_%s'%dom]
else :
  ...
  fileo.comment = 'The SZ is computed as the spatial integration of vosaline'
  ...
  depthaxis.units='m'
  for dom in domains :
    hc[dom] = fileo.createVariable('SZ_%s'%dom,'f',('time','level'))
    hc[dom].units = 'pss'
    hc[dom].long_name = 'averaged salinity in %s'%dom
...
for year in range(yrdeb,yrfin+1) :
  ...
  tab,timex = ct.read(filein,'vosaline',years=range(year,year+1))
  #
