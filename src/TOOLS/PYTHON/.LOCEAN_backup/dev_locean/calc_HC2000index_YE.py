...
depthindicemin, depthindicemax = ct.coord2indice(depth,(0,2000))
...
if os.path.exists(fileout) :
  ...
  if 'HC2000_AMO' in fileo.variables.keys() :
  ...
  for dom in domains :
    hc[dom] = fileo.variables['HC2000_%s'%dom]
else :
  ...
  for dom in domains :
    hc[dom] = fileo.createVariable('HC2000_%s'%dom,'f',('time',))
    hc[dom].units = '1e22 J'
    hc[dom].long_name = 'Heat content [0-2000m] in %s'%dom
