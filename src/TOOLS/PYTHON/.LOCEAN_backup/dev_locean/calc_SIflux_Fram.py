...
# ========================
#
# moce : Masque de l'océan
#       --> array dans laquelle, 0=Land, 1=Ocean, 2= Section d'intéret
#
# l : Definition de la section 
#     -->LISTE Contenant les indices des mailles de la section d'intéret en
#        grille NEMO1
#
# sec : même chose que l, mais sous forme d'array de dimension (N,2),
#       avec N le nombre de maille dans la section
#
# extr : fonction qui extrait la section d'intéret d'un champs 3D (Nx,Ny,Nz)
#        d'un input de dim (Nt,Nz,Ny,Nx) ressort
#        un output de dim (Nt,N,Nz) N=Nbe de mailles de sec
#        ==> ressort des sections verticales
#
# extr2D : même chose pour champs 2D (Ny,Nx)
#
...
#        la section d'intéret (grille U)
#
# nlev : Contient les profondeurs z (en m) des niveaux de la grille
#        dim=(42), 42 niveaux
#
# o1 : Contient la différence d'indice en x et en y d'une maille à l'autre
#      dans la section d'intéret. dim (N,2)
#      Dans le cas de Fram, cette section est zonale, i.e. suivant un indice
#      constant i=271
#      Alors o1[:,0]=0
#      Et on prend toutes les mailles d'affiler sur un même parallène
#      Alors o1[:,1]=1
#      Rq : dans le cas d'une section plus complexe on ferait N.diff(sec)
#
# oo3D : Même chose répliqué sur les 42 niveaux verticaux
#        Sert au calcule du transport.
#        En effet : sur une maille, le débit zonale : u*dy est à prendre
#        en compte
...
def read(dirin,filein,varname,years=None) :
  ...
  if years is not None :
    tdeb = datetime.datetime(years[0],1,1) # 1er Jan de la 1ere ann�e
    tfin = datetime.datetime(years[-1],12,31) #31/12 de la derni�re ann�e
...
# années disponibles
# ~~~~~~~~~~~~~~~~~~~
yrdeb = int(exp.year[0])
yrfin = int(exp.year[1])
...
for year in range(yrdeb,yrfin+1):
 ...
 #
 ui, timeval, timeu, timecal = ag.read(exp.loc('I'),exp.fname('iicevelu'),'iicevelu',years=range(year,year+1))
 uui = extr2D(ui,sec)
 #
 vi,timeval, timeu, timecal = ag.read(exp.loc('I'),exp.fname('iicevelv'),'iicevelv',years=range(year,year+1))
 vvi = extr2D(vi,sec)
 #
 hi,timeval, timeu, timecal = ag.read(exp.loc('I'),exp.fname('iicethic'),'iicethic',years=range(year,year+1))
 hhi = extr2D(hi,sec)
 #
 ...
 for mth in range(12):
  ...
  tmpvi=vvi[mth]
  tmphi=hhi[mth]
  fvi=((tmpui*oo3D[0,:,0]*re2u[0,:] + tmpvi*oo3D[0,:,1]*re1v[0,:])*tmphi).sum()
  fai=((tmpui*oo3D[0,:,0]*re2u[0,:] + tmpvi*oo3D[0,:,1]*re1v[0,:])).sum()
  #
  # Transport cumul� sur le mois en km3 et km2  respectivement:
  if mth<11 :
