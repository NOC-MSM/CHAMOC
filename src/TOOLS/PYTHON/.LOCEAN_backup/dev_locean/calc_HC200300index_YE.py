...
depthindicemin, depthindicemax = ct.coord2indice(depth,(200,300))
...
if os.path.exists(fileout) :
  ...
  if 'HC200300_AMO' in fileo.variables.keys() :
  ...
  for dom in domains :
    hc[dom] = fileo.variables['HC200300_%s'%dom]
else :
  ...
  for dom in domains :
    hc[dom] = fileo.createVariable('HC200300_%s'%dom,'f',('time',))
    hc[dom].units = '1e22 J'
    hc[dom].long_name = 'Heat content [200-300m] in %s'%dom
