...
depthindicemin, depthindicemax = ct.coord2indice(depth,(500,700))
...
if os.path.exists(fileout) :
  ...
  if 'HC511_AMO' in fileo.variables.keys() :
  ...
  for dom in domains :
    hc[dom] = fileo.variables['HC511_%s'%dom]
else :
  ...
  for dom in domains :
    hc[dom] = fileo.createVariable('HC511_%s'%dom,'f',('time',))
    hc[dom].units = '1e22 J'
    hc[dom].long_name = 'Heat content [level 511 m] in %s'%dom
