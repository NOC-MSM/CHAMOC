...
if os.path.exists(fileout) :
  ...
  fileo.comment = 'The AMO is computed as the spatial averaged of the annual SST in the 80W-0E,0N-60N domain'
  time = fileo.createDimension('time',None)
  times = fileo.createVariable('time_counter','f',('time',))
  amo = fileo.createVariable('AMO','f',('time',))
  #amorev = fileo.createVariable('AMOrev','f',('time',))
  amo.units = 'degC'
  #amorev.units = 'degC'
  amo.long_name = 'Atlantic Multidecadal Oscillation index'
...
for year in range(yrdeb,yrfin+1) :
  ...
  #print timex.year()
  #
  #tab_glo  = regional_average(tab,'GLOOCEAN60',operation='average')
  tab_amo = ct.regional_average(tab,'AMO',operation='average')
  #
  # Annual mean
