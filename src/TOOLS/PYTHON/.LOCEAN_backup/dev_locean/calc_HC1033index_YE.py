...
depthindicemin, depthindicemax = ct.coord2indice(depth,(1000,1400))
...
if os.path.exists(fileout) :
  ...
  if 'HC1033_AMO' in fileo.variables.keys() :
  ...
  for dom in domains :
    hc[dom] = fileo.variables['HC1033_%s'%dom]
else :
  ...
  for dom in domains :
    hc[dom] = fileo.createVariable('HC1033_%s'%dom,'f',('time',))
    hc[dom].units = '1e22 J'
    hc[dom].long_name = 'Heat content [1000-1400 m] in %s'%dom
