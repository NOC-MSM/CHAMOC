...
depthindicemin, depthindicemax = ct.coord2indice(depth,(300,400))
...
if os.path.exists(fileout) :
  ...
  if 'HC300400_AMO' in fileo.variables.keys() :
  ...
  for dom in domains :
    hc[dom] = fileo.variables['HC300400_%s'%dom]
else :
  ...
  for dom in domains :
    hc[dom] = fileo.createVariable('HC300400_%s'%dom,'f',('time',))
    hc[dom].units = '1e22 J'
    hc[dom].long_name = 'Heat content [300-400m] in %s'%dom
