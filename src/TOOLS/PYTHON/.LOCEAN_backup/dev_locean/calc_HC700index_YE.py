...
if len(sys.argv)<3 :
  ...
  fin = sys.argv[2]
#
DIAG_ID = 'HC700index_YE'

# Repertoires des inputs :
# ~~~~~~~~~~~~~~~~~~~~~~~
exp = dexp[EXP_ID]
domains = ['AMO','GLOOCEAN','PACNORTH30']
...
depthindicemin, depthindicemax = ct.coord2indice(depth,(0,700))
...
if os.path.exists(fileout) :
  ...
  if 'HC700_AMO' in fileo.variables.keys() :
  ...
  for dom in domains :
    hc[dom] = fileo.variables['HC700_%s'%dom]
else :
  ...
  for dom in domains :
    hc[dom] = fileo.createVariable('HC700_%s'%dom,'f',('time',))
    hc[dom].units = '1e22 J'
    hc[dom].long_name = 'Heat content [0-700m] in %s'%dom
