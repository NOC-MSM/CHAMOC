...
# Modified: no 01
# CONSTRUCTION DES SERIES TEMPORELLES SSTtropglo, SSTtropatl, SSTtroppac
#======================================================================== 
#global AG
...
if os.path.exists(fileout) :
  ...
  ssttropglo = fileo.variables['SSTtropglo']
  ssttropatl = fileo.variables['SSTtropatl']
  ssttroppac = fileo.variables['SSTtroppac']
  times = fileo.variables['time_counter']
  if 'SSTtropglo' in fileo.variables.keys() :
  ...
  ssttropglo = fileo.createVariable('SSTtropglo','f',('time',))
  ssttropatl = fileo.createVariable('SSTtropatl','f',('time',))
  ssttroppac = fileo.createVariable('SSTtroppac','f',('time',))
  ssttropglo.units = 'degC'
  ssttropatl.units = 'degC'
  ssttroppac.units = 'degC'
  ssttropglo.long_name = 'Tropical band(20S-20N) sea surface temperature average'
  ssttropatl.long_name = 'Atlantic tropical band(20S-20N) sea surface temperature average'
  ssttroppac.long_name = 'Pacific tropical band(20S-20N) sea surface temperature average'
  #times.units=timeu
#
#
for year in range(yrdeb,yrfin+1) :
  ...
  #
  tab_glo  = ct.regional_average(tab,'TROP20',operation='average')
  tab_atl = ct.regional_average(tab,'ATLTROP20',operation='average')
  tab_pac = ct.regional_average(tab,'PACTROP20',operation='average')

  # Writing in netcdf file
  # ---------------------- 
  if ssttropglo.shape[0]==0 :
    ssttropglo[:]=tab_glo.filled()
    ssttropatl[:]=tab_atl.filled()
    ssttroppac[:]=tab_pac.filled()
    times[:]=timex.num()
  else :
    i1=len(ssttropglo)
    ssttropglo[i1:i1+12]=tab_glo.filled()
    ssttropatl[i1:i1+12]=tab_atl.filled()
    ssttroppac[i1:i1+12]=tab_pac.filled()
    times[i1:i1+12]=timex.num()
  del tab_glo, tab_atl, timex.values
