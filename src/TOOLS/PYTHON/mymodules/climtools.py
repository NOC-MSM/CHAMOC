...
# --------------------------
# Author : A. Germe
# moving : after Jan. 2017
# version 0
# ---------------------------
# Cette version repart de 0, en se basant sur la version climtools que
# j'avais à l'IPSL mais en tentant de faire qqch de plus générique
# Dans ce package, il n'y a que des fonctions qui ne dépendent pas de
# d'info particulière sur les expé disponibles (direxp.py) etc ...
# =========================================================================
import sys, os
...
def readfield(filein,varname,years=None) :
  ...
  lat = axis(latvar[...],units=latvar.units,id='latitude')
  f.close()
...
# -----------------------------------------------------------------------
def selectseason(tab,axt,season='JFM',postime=0) :
  """
      selectionne la saison d'un champs
    . postime = position de la dimension temporelle dans tab
  """
  indices = N.array(range(len(axt.month())))
  isinseason = N.array([x in dseason[season] for x in axt.month()])
  #
  tabout = N.take(tab,indices[isinseason],axis=postime)
...
# -----------------------------------------------------------------------
def seasonalmean(tab,axt,season='JFM',postime=0) :
  """ tabseason,timeseason=seasonalmean(tab,axt,season='JFM',postime=0)

      Compute the seasonal mean on the yearly basis. 
      !!! WARNING : Means that if you choose cross-year season like DJF, you 
      will end up with JF-D average of the same year, instead of cross-year
      D-JF average !!!
      ...
      Inputs :
       ...
       tab : array
       axt : time axis
       season : string such as 'JFM', 'JJA', 'FM', 'ALL' ....etc

      Outputs :

       tabnew : array, time dimension 1st
       timenew : time axis, arbitrary takes the 1st of January of each year.
                 
  """
  tabseason,timeseason=selectseason(tab,axt,season=season,postime=postime)
  years=list(set(timeseason.year()))
  indices = N.array(range(len(timeseason.year())))
  tabnew=list()
  for year in years :
    isinyear = timeseason.year() == year
    tabtmp   = N.take(tabseason,indices[isinyear],axis=postime)
    tabnew.append(MA.average(tabtmp,axis=postime))
  #
  tabnew  = N.array(tabnew)
  timenew_value = N.array([datetime.datetime(x,1,1) for x in years])

#  isinseason = N.array([x in dseason[season] for x in axt.month()])
  timenew = timeaxis(timenew_value,units=axt.units,calendar=axt.calendar,origin=axt.origin)
  return tabnew,timenew
# -----------------------------------------------------------------------
def find_nearest(tab,value):
    idx = (ma.abs(tab-value)).argmin()
    return idx
# -----------------------------------------------------------------------
def eof_eco(M,wt=None,detrend='F'):
  """ varexp,eofs,pcs=eof_eco(M,wt=None)
      
      Inputs :
      
       M : array with Nt x Np shape
       wt : array, same dimension as M 
            weight to apply to M

      Outputs :

       varexp   : array : percentage of explained variance
       evectors : eigen vectors (normalized)
       proj     : projection of M on eigen vectors
  """
  # weights
  if wt is not None :
    assert M.shape==wt.shape, "M and wt don\'t have the same shape"
    M = M*wt

  
  # Matrice en anomalies autour de la moyenne ou du trend
  if detrend=='T' :
    Mcenter = scipy.signal.detrend(M,axis=0)
  else :
    Mcenter = M - M.mean(axis=0)
  #
  Nt,Np = Mcenter.shape

  # compute eofs
  if Nt>Np :
    MCOV = N.cov(Mcenter.transpose())     # shape : Np x Np
    evalues,evectors = N.linalg.eig(MCOV) # evector shape : Np x Np
    proj = N.dot(Mcenter,evectors)        # shape : Nt x Np
    #
    eofs = evectors
    pcs  = proj
  else :
    MCOV = N.cov(Mcenter)                 # shape : Nt x Nt
    evalues,evectors = N.linalg.eig(MCOV) # evector shape : Nt x Nt
    proj = N.dot(Mcenter.transpose(),evectors) # shape : Np x Nt
    #
    eofs = proj
    pcs  = evectors


  varexp = 100 * evalues/sum(evalues)   
  return varexp,eofs,pcs
#
